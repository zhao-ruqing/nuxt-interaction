...
## 16. 开发实现记录（2026-07-14）

本轮已在保留饮品商城全部功能的基础上，完成行鉴桌面端首个可运行版本：

- MySQL 连接参数完成环境变量化，本机使用 `D:\Environment\MySQL` 下的 MySQL 8.4.9；
- 新增 `server/sql/xingjian.sql` 与 `server/sql/xingjian-rbac.sql`，建立城市、点位、路线、打卡、积分、徽章、活动、模拟支付、积分商城、好友、团队和设置等业务表；
- 已将 SQL 导入 `nuxt_interaction` 数据库，并初始化上海、杭州、文化点位、活动和积分商品数据；
- 完成城市、点位、路线完成、打卡、积分、徽章、个人/好友/团队排行榜、活动报名、模拟支付、积分兑换、好友申请和团队 API；
- 打卡使用 MySQL 事务和每日唯一约束防止重复发放积分；
- 桌面端打卡支持浏览器定位，后台可通过 `location_check_enabled` 开关启用服务端距离围栏校验；
- 积分兑换使用账户行锁、商品行锁和事务防止积分透支与库存超兑；
- 新增桌面端行鉴前台：首页、城市、点位、路线、活动、排行榜、积分商城、好友团队、订单记录和个人中心；
- 新增后台城市、点位、路线、活动、积分商品、兑换订单、用户角色和系统设置页面，原饮品商品管理、饮品商城和支付页面保持不变；
- 普通用户与管理员通过 `users.role` 隔离，后台页面和后台 API 均执行管理员权限校验；
- 行鉴后台新增、修改、订单发货、角色调整和系统设置变更均接入现有审计日志；
- 活动报名确认采用模拟支付，支付成功后确认报名并发放活动积分；虚拟积分商品兑换后自动完成，实物商品进入待发货状态；
- 生产构建已通过，并完成“注册 → 登录 → 打卡 → 重复打卡拦截 → 活动报名 → 模拟支付”的 HTTP 冒烟验证。

后续生产化工作主要是接入真实地图围栏、真实扫码支付渠道、图片/文件存储、短信或第三方登录，以及根据实际数据量引入排行榜快照任务。
