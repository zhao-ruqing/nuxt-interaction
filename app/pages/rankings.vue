<template>
  <div class="xj-page">
    <span class="xj-kicker">MYSQL LIVE RANKING</span><h1 class="xj-title">探索排行</h1><p class="xj-lead">榜单直接基于 MySQL 积分账户、打卡和团队成员聚合，后续数据量增长后可切换为快照表。</p>
    <div class="xj-toolbar"><button class="xj-button" :class="{'xj-button--ghost':tab!=='user'}" @click="tab='user'">个人榜</button><button class="xj-button" :class="{'xj-button--ghost':tab!=='friend'}" @click="showFriendRanking">好友榜</button><button class="xj-button" :class="{'xj-button--ghost':tab!=='team'}" @click="tab='team'">团队榜</button></div>
    ...
    <div v-else-if="tab==='friend'" class="xj-panel"><table class="xj-table"><thead><tr><th>RANK</th><th>好友</th><th>累计积分</th><th>当前积分</th><th>打卡数</th></tr></thead><tbody><tr v-for="item in friends" :key="item.userId"><td class="rank">#{{ String(item.rank).padStart(2,'0') }}</td><td>{{ item.username }}</td><td>{{ item.totalEarned }}</td><td>{{ item.balance }}</td><td>{{ item.checkinCount }}</td></tr></tbody></table></div>
...
const [{ data }, { data: teamData }] = await Promise.all([useFetch<any>('/api/rankings'), useFetch<any>('/api/rankings/teams')])
const rankings = computed(() => data.value?.data || [])
