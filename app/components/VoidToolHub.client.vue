  ...
  >
    ...
    <TransitionGroup name="orb">
      <button
        ...
        :key="orb.key"
        class="tool-hub__orb"
        :class="[`tool-hub__orb--${orb.key}`, { 'tool-hub__orb--active': orb.active }]"
        type="button"
        :title="orb.label"
        :style="orbStyle(i)"
    ...
    <Transition name="panel">
      ...
      >
        ...
        >
          ...
          <button class="tool-hub__clear" type="button" title="清空对话" @pointerdown.stop @click="clearMessages">
            清空
          </button>
        </div>
          ...
          >
            <div class="tool-hub__bubble">
              <template v-if="msg.loading && !msg.content">
                <span class="tool-hub__typing">
                  <span /><span /><span />
                </span>
              </template>
              <template v-else>
                <p v-if="msg.content" class="tool-hub__text">{{ msg.content }}</p>
                ...
                <div v-if="msg.action" class="tool-hub__action">
                  ...
                  <p v-if="msg.action.action === 'ORDER' && orderStatusText" class="tool-hub__action-status">
                    {{ orderStatusText }}
                  </p>
                </div>
              </template>
...
const fanAngles = computed(() => {
  ...
  const preference = onRight
    ? (onTop
        ? ['down-left', 'left', 'down', 'up-left', 'up', 'down-right', 'right', 'up-right']
        : ['up-left', 'left', 'up', 'down-left', 'down', 'up-right', 'right', 'down-right'])
    : (onTop
        ? ['down-right', 'right', 'down', 'up-right', 'up', 'down-left', 'left', 'up-left']
        : ['up-right', 'right', 'up', 'down-right', 'down', 'up-left', 'left', 'down-left'])
...
.tool-hub__orb {
  ...
  transition: background 0.2s, border-color 0.2s, color 0.2s, box-shadow 0.2s, left 0.22s ease, top 0.22s ease, opacity 0.22s ease, transform 0.22s ease;
